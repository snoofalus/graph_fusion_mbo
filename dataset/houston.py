...
#CREATES ONE COMBINED SET FROM DATA AND MASK DIRS
def get_houston(root):
	...
	def npy_loader(path):
		...
		#transform to tensor once either here or in ToTensor
	...
	#dataset = custom.SegmentationDataset(root_image=data_dir, root_mask=mask_dir, loader=npy_loader, image_transform=image_transform, mask_transform=mask_transform)
	dataset = custom.SegmentationDataset(root_image=data_dir, root_mask=mask_dir, loader=npy_loader, image_transform=None, mask_transform=None)
	
	print('Total images: {}'.format(len(dataset)))
	
	return dataset

#SPLITS ROOT INTO LIST OF PATHS AND SPLITS INTO TRAIN + VAL
def get_houston_trainval(root):
	...
	print('Train images: {} Val images: {}'.format(len(train_dataset), len(val_dataset)))
